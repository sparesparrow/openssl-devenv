...
mkdir -p openssl-devenv/bootstrap
mkdir -p openssl-devenv/.vscode
mkdir -p openssl-devenv/scripts
mkdir -p openssl-devenv/docs
...
if [[ "$TRACK" == "B" ]]; then
    ...
    cat > openssl-devenv/bootstrap/openssl-conan-init.py << 'EOF'
#!/usr/bin/env python3
"""
...
EOF
    ...
    cat > openssl-devenv/.vscode/extensions.json << 'EOF'
{
  "recommendations": [
    "ms-vscode.cpptools",
...
EOF
    ...
    cat > openssl-devenv/.vscode/launch.json << 'EOF'
{
  "version": "0.2.0",
  "configurations": [
    {
...
EOF
    ...
    cat > openssl-devenv/scripts/update-intellisense.py << 'EOF'
#!/usr/bin/env python3
"""
...
EOF
    ...
    cat > openssl-devenv/docs/getting-started.md << 'EOF'
...
EOF
    ...
    echo ""
    echo "Created files:"
    echo "  - openssl-devenv/bootstrap/openssl-conan-init.py"
    echo "  - openssl-devenv/.vscode/extensions.json"
    echo "  - openssl-devenv/.vscode/launch.json"
    echo "  - openssl-devenv/scripts/update-intellisense.py"
    echo "  - openssl-devenv/docs/getting-started.md"
    echo ""
    echo "Next: Test the bootstrap script"
    echo "  cd /tmp && python3 ~/projects/openssl-devenv/openssl/openssl-devenv/bootstrap/openssl-conan-init.py --dev"
